...
module.exports.noPerms = async (message, perm, lang) => {
    ...
    message.delete();
     ...
     if(lang == 'fr'){
       noPermMSG = `**${cfg.emojis.animated.error} Vous n'avez pas la permission: ${perm}**`;
     } else if(lang == 'en'){
       noPermMSG = `**${cfg.emojis.animated.error} You doesn't have permission: ${perm}**`;
     }

    message.channel.send(noPermMSG)
}
...
module.exports.noMultiPerms = async (message, perm, lang) => {
    ...
    message.channel.send(`**${cfg.emojis.animated.error} Vous n'avez pas les permission: ${perm}**`)
}
...
module.exports.disabled = async (message) => {
    ...
    message.channel.send(`**${cfg.emojis.animated.error} Cette commande est actuellement désactivée !**`).then(m => m.delete({
        timeout: 3000
    }));
}
